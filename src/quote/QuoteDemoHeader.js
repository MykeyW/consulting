...
export default function QuoteDemoHeader({ styleIds, styleId, setStyleId }) {
  const { lang, setLang, t } = useLangTheme();

  return (
    <header className="border-b border-slate-800/60 bg-slate-950/80 backdrop-blur">
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-4 flex flex-col gap-4">
        {/* Top row: brand + title + language */}
        ...
        {/* Bottom row: style pills + back-to-site */}
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          {/* Style presets */}
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
            <span className="text-xs uppercase tracking-wide text-slate-400">
              {t.booking_style_label || "Style"}
            </span>
          ...
          <div className="md:ml-auto">
            ...
            >
              ...
              <span>{t.quote_back_to_site}</span>
            </Link>
          </div>
        </div>
      </div>
